<aside class="main-sidebar sidebar-dark-primary elevation-4">
          ...
          <li class="nav-item dropdown">
            <a href="./index.php?page=messages" class="nav-link nav-messages">
              ...
              <p>
                ...
                $unread_count = 0;
                $result = $conn->query("SELECT COUNT(*) as cnt FROM messages WHERE status = 0");
                ...
                if($unread_count > 0): 
                ?>
                <span class="right badge badge-danger"><?php echo $unread_count; ?></span>
                <?php endif; ?>
              </p>
            </a>
          </li>
  ...
  <script>
      ...
      function updateUnreadCount() {
          $.ajax({
              url: 'ajax.php?action=unread_count',
              method: 'GET',
              success: function(count) {
                  var $badge = $('.nav-messages .badge');
                  if(parseInt(count) > 0) {
                      if($badge.length) {
                          $badge.text(count);
                      } else {
                          $('.nav-messages p').append('<span class="right badge badge-danger">' + count + '</span>');
                      }
                  } else {
                      $badge.remove();
                  }
